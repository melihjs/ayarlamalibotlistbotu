...
exports.run = async (client, message, args) => {
  ...
  if(botid) {
    ...
    if(client.channels.cache.get(onayred).send('<:'+emoji.name1+':'+client.emojis.cache.get(emoji.yes)+"> <@"+botid+"> adlı bot <@"+message.author.id+"> tarafından onaylandı!"));
    message.reply('<:'+emoji.name1+':'+client.emojis.cache.get(emoji.yes)+"> <@"+botid+"> adlı bot onaylandı!")
    db.delete(`bot.id.${botid}`)
    }
  }
};

exports.config = { name:"botonayla", aliases:["bot-onayla"] };